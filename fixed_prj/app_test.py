...
def view_schedule():
    ...
    for sched in schedules:
        ...
        # flight_results를 딕셔너리 리스트로 변환
        flight_results = [dict(r) for r in results]

        # 각 항공권 결과에 대해 전일 평균가 대비 변동률 계산
        for flight in flight_results:
            try:
                # 현재 가격과 fetch_time 파싱
                current_price = int(flight['flight_cost'].replace('₩', '').replace(',', '').strip())
                ...
                # 동일 항공권 정보에 대해 전일 평균 가격 계산
                conn2 = get_db_connection()
                row = conn2.execute('''
                    SELECT AVG(CAST(REPLACE(flight_cost, "₩", "") AS INTEGER)) as avg_price
                    FROM flight_results
                    WHERE schedule_id = ?
                      AND flight_name1 = ?
                      AND flight_time_s1 = ?
                ...
                if row and row['avg_price']:
                    avg_previous_hour = row['avg_price']
                    avg_previous_hour_full = avg_previous_hour * 1000 
                    variation = (current_price - avg_previous_hour_full) / avg_previous_hour_full * 100        
                ...
                # print("  flight_time_s1:", flight['flight_time_s1'])
                # print("  current_time:", current_time)
                # print("  current_price:", current_price)
                # print("  Query Range:", previous_hour_start, "~", previous_hour_end)
                ...
                # 데이터 부족이나 예외 발생 시 변동률 정보 없이 None 처리
                flight['variation'] = None
...
def view_flight_trend():
    ...
    df['time'] = pd.to_datetime(df['time'])  # 혹시 time이 문자열이면 변환
...
def view_lowest_price_trend():
    ...
    if resampled_times:
        ...
        end_time = current_time + timedelta(hours=0)
        plt.xlim(start_time, end_time)
